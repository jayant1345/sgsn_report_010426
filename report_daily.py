...
def _build_mar26(wb, year, month, month_name, report_day,
    ...
    for d in range(1, days+1):
        ...
        if cutover_day is not None and d >= cutover_day:
            ...
            daily[d]['dt_ul'] = max(0.0, nib_ul_gb - daily[d]['nok_ul'] - nok4g_ul) * 0.7
            daily[d]['dt_dl'] = max(0.0, nib_dl_gb - daily[d]['nok_dl'] - nok4g_dl) * 0.7
        else:
            daily[d]['dt_ul'] = max(0.0, nib_ul_gb - daily[d]['nok_ul'] - daily[d]['zte_ul']) * 0.7
            daily[d]['dt_dl'] = max(0.0, nib_dl_gb - daily[d]['nok_dl'] - daily[d]['zte_dl']) * 0.7
